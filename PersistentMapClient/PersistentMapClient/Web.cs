...
namespace PersistentMapClient {
    public static class Web {
        ...
        public static bool PostMissionResult(MissionResult mresult) {
            try {
                string URL = Fields.settings.ServerURL + "warServices/Mission/V2/?employer=" + mresult.employer + "&target=" + mresult.target
                    + "&systemName=" + Uri.EscapeDataString(mresult.systemName) + "&mresult=" + mresult.result.ToString() + "&difficulty=" + mresult.difficulty;
                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(URL);
                request.ContentType = "application/json; charset=utf-8";
                request.Method = "GET";
                HttpWebResponse response = request.GetResponse() as HttpWebResponse;
